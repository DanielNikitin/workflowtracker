...
const WorkList = ({ onSuccess, onProcess, onError, userId }) => {
  ...
  useEffect(() => {
    const fetchUserGroup = async () => {
      try {
        const response = await fetch(`https://api2.dcg.ee:3009/api/user/group/${userId}`);
        if (!response.ok) {
  ...
  const fetchWorks = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://api2.dcg.ee:3009/api/works?userId=${clientToUse}&month=${formattedMonth}&year=${selectedYear}`);
  ...
  const handleSave = async () => {
    try {
      const response = await fetch(`https://api2.dcg.ee:3009/api/works/${editingWork.id}`, {
        method: 'PUT',
        headers: {
